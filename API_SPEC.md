...
| `DELETE` | `/member/:id` | — | `{ success, memberId }` | Admin removal: deletes the member, ledger/visit/redemption history, and the pass's device registrations + update state. Deleted passes stop updating. |
| `GET` | `/members?search=&limit=` | — | `{ members, count }` | Admin search across name/email (service-auth only). |
